...
class Schedule {
    ...
    // REVERT: Admin submissions are approved, but Faculty are 'pending'
        ...
        foreach ($result as $row) {
            ...
            if (!isset($grouped[$userId])) {
                $grouped[$userId] = [
                    ...
                    'daily_schedules' => [], // CHANGED from 'schedules' to match view
                    'stats' => [
                        'total_hours' => 0,
                        'day_totals' => [] 
                    ]
            ...
            $grouped[$userId]['daily_schedules'][] = $row; // CHANGED from 'schedules'
            
            $duration = (strtotime($row['end_time']) - strtotime($row['start_time'])) / 3600;
            
            if (!isset($grouped[$userId]['stats']['day_totals'][$day])) {
                $grouped[$userId]['stats']['day_totals'][$day] = 0;
            }
            $grouped[$userId]['stats']['day_totals'][$day] += $duration;
