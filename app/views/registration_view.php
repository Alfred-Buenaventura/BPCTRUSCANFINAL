...
function connectWebSocket() {
    // connecting to the local c# bridge app
    socket = new WebSocket("ws://127.0.0.1:8080");

    socket.onopen = () => {
        console.log("✅ WebSocket established. Verifying physical scanner handle...");
        // 1. checking if the usb scanner is actually plugged in
        socket.send(JSON.stringify({ command: "check_hardware" }));
        // 2. syncing up the finger templates
    ...
    socket.onmessage = (event) => {
        try {
            ...
            console.log("📨 Message from Bridge:", data);

            if (data.type === "hardware_status") {
                if (data.connected === true) {
                    isDeviceConnected = true;
                    deviceStatusText.textContent = "Hardware Ready";
                    deviceStatusIcon.className = "fa fa-check-circle text-emerald-500";
                    deviceStatusContainer.style.backgroundColor = "#f0fdf4";
                    if (openModalBtn) openModalBtn.disabled = false;
                } else {
                    isDeviceConnected = false;
                    deviceStatusText.textContent = "Scanner Not Found (Check USB)";
                    deviceStatusIcon.className = "fa fa-exclamation-triangle text-red-500";
                ...
                const stepCircle = document.getElementById(`scanStep${data.step}`);
                if (stepCircle) stepCircle.classList.add('active-step');
                scanStatus.textContent = data.message;
            }

            else if (data.status === "success") {
                console.log("✅ Identity captured. Uploading to server...");
                scanStatus.textContent = "Finalizing registration...";
                saveToServer(data.template);
            }

            else if (data.status === "error") {
                alert("Scanner Error: " + data.message);
                resetScanUI();
            }
    ...
    socket.onclose = () => {
        isDeviceConnected = false;
        console.warn("🔌 Connection to Bridge Service lost.");

        deviceStatusText.textContent = "Scanner Service Offline";
        deviceStatusIcon.className = "fa fa-power-off text-gray-400";
        deviceStatusContainer.style.backgroundColor = "#f3f4f6";
    ...
    socket.onerror = (err) => {
        console.error("WebSocket Fault: Check if ZKTecoBridge.exe is running.");
    };
}

async function saveToServer(template) {
    const fingerName = document.getElementById('fingerSelector').value;
    // using an absolute path so it works on live servers
    const apiUrl = '/api/register_finger.php'; 
    
    try {
        scanStatus.textContent = "Sending template to server...";
        ...
        if (result.status === "success") { 
            scanStatus.textContent = "Successfully Registered!"; 
            // redirect to the finish screen so it doesn't loop back to scanning
            setTimeout(() => window.location.href = 'complete_registration.php', 1500);
        } else {
...
document.addEventListener('DOMContentLoaded', () => { 
    connectWebSocket(); if(openModalBtn) 
    openModalBtn.addEventListener('click', openInstructionModal); 
});
