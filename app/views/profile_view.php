                    ...
                    <div class="card" style="margin-top: 2rem; border-left: 5px solid #10b981;">
                            <div class="card-body">
                                <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 1.5rem;">
                                    <div style="background: #ecfdf5; color: #10b981; width: 45px; height: 45px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;">
                                        <i class="fa-solid fa-qrcode"></i>
                                    </div>
                                    <div>
                                        <h3 style="margin: 0; font-size: 1.25rem;">Attendance QR Code</h3>
                                        <p style="margin: 0; font-size: 0.85rem; color: #64748b;">Emergency backup for ID printing</p>
                                    </div>
                                </div>

                                <div style="text-align: center; background: #f8fafc; padding: 30px; border-radius: 12px; border: 2px dashed #e2e8f0;">
                                    <?php if (!empty($user['qr_token'])): ?>
                                        <img src="public/uploads/qr_<?= $user['qr_token']; ?>.png" 
                                            alt="Attendance QR" 
                                            onerror="this.src='public/assets/images/default_qr.png'"
                                            style="width:200px; border:1px solid #ddd; background: #ffffff; padding: 10px; border-radius: 8px;">

                                        <div style="margin-top: 20px;">
                                            <p style="font-size: 0.9rem; color: #475569; margin-bottom: 15px;">
                                                <strong>Note:</strong> You can download and print this to attach to your institutional ID.
                                            </p>
                                            
                                            <a href="public/uploads/qr_<?= $user['qr_token']; ?>.png" 
                                            download="BPC_Attendance_QR_<?= $user['faculty_id']; ?>.png" 
                                            class="btn btn-primary">
                                            <i class="fa-solid fa-download"></i> Download QR Code
                                            </a>
                                        </div>
                                    <?php else: ?>
                                        <p class="text-error">QR Code not found. Please contact MIS to reset your token.</p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        <?php if ($isAdmin): ?>
                        <div class="card" style="margin-top: 2rem; border-left: 5px solid #6366f1;">
                            <div class="card-body">
                                <div style="display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 15px;">
                                        <div style="background: #eef2ff; color: #6366f1; width: 45px; height: 45px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;">
                                            <i class="fa-solid fa-key"></i>
                                        </div>
                                        <div>
                                            <h3 style="margin: 0; font-size: 1.25rem;">Terminal Access Control</h3>
                                            <p style="margin: 0; font-size: 0.85rem; color: #64748b;">Manage the security PIN for the QR Scanner interface</p>
                                        </div>
                                    </div>
                                    <button type="button" onclick="openTerminalPinModal()" class="btn btn-primary">
                                        <i class="fa-solid fa-gear"></i> Set PIN
                                    </button>
                                </div>
                            </div>
                        </div>

                        <div id="terminalPinModal" class="modal">
                            <div class="modal-content" style="max-width: 400px;">
                                <div class="modal-header">
                                    <h3 style="margin: 0; font-size: 1.1rem;">Terminal PIN Setup</h3>
                                    <span class="close-btn" onclick="closeTerminalPinModal()">&times;</span>
                                </div>
                                <form method="POST" action="profile.php">
                                    <?php csrf_field(); ?>
                                    <div class="modal-body" style="padding: 25px; text-align: center;">
                                        <p style="font-size: 0.9rem; color: #64748b; margin-bottom: 20px;">
                                            This PIN is required to switch from the Fingerprint scanner to the QR Scanner interface.
                                        </p>
                                        <div class="form-group">
                                            <label style="font-weight: 700; display: block; margin-bottom: 10px;">Current Terminal PIN: <span style="color: #6366f1;"><?= $terminal_pin ?></span></label>
                                            <input type="text" name="new_terminal_pin" placeholder="Enter 4-digit PIN" 
                                                maxlength="4" pattern="\d{4}" required
                                                style="font-size: 2rem; text-align: center; letter-spacing: 10px; width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 12px;">
                                        </div>
                                    </div>
                                    <div class="modal-footer" style="padding: 15px 20px;">
                                        <button type="button" class="btn btn-secondary" onclick="closeTerminalPinModal()">Cancel</button>
                                        <button type="submit" name="update_terminal_pin" class="btn btn-primary">Save New PIN</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
function downloadQrCode() {
    ...
    link.href = img.src + '&download=1'; 
    link.download = "BPC_Attendance_QR.png";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}

window.openTerminalPinModal = function() { 
    document.getElementById('terminalPinModal').style.display = 'flex'; 
}
window.closeTerminalPinModal = function() { 
    document.getElementById('terminalPinModal').style.display = 'none'; 
}
